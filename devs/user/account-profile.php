...
if ($user_id) {
    ...
    // Eğer kullanıcı bulunamadıysa varsayılan değerler ata
    if (!$user) {
        $user = [
            'username' => '',
            'email' => '',
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['profile_photo'])) {
    ...
    if (!in_array($file['type'], $allowed_types)) {
        ...
        if (move_uploaded_file($file['tmp_name'], $target_path)) {
            ...
            if ($stmt->execute()) {
                $success_message = "Profil fotoğrafı başarıyla güncellendi.";
                $profile_photo = $photo_path;
            } else {
                $error_message = "Profil fotoğrafı güncellenirken bir hata oluştu.";
            }
        } else {
            $error_message = "Dosya yüklenirken bir hata oluştu.";
        }
