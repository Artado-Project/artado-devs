...
$default_profile_photo = 'https://artado.xyz/assest/img/artado-yeni.png'; // URL olarak kalsın
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['profile_photo']) && $_FILES['profile_photo']['error'] === UPLOAD_ERR_OK) {
    ...
    if (!in_array($file['type'], $allowed_types)) {
        $error_message = "Sadece JPEG, PNG ve GIF formatında dosyalar yükleyebilirsiniz.";
    } elseif ($file['size'] > $max_size) {
        $error_message = "Dosya boyutu en fazla 5MB olabilir.";
    } else {
        // Hedeflenen yeni dizin (user klasörüne göre)
        $upload_dir = '../public/uploads/avatars/'; 
        if (!file_exists($upload_dir)) {
            if (!mkdir($upload_dir, 0755, true)) { 
                $error_message = "Profil fotoğrafı yükleme dizini oluşturulamadı.";
                error_log("Avatar yükleme dizini oluşturulamadı: " . $upload_dir);
                goto end_photo_upload; 
            }
        }

        $file_extension = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION));
        $file_name = uniqid('avatar_') . '.' . $file_extension; 
        $target_path = $upload_dir . $file_name; // Tam sunucu yolu (ör: ../public/uploads/avatars/avatar_123.jpg)

        if (move_uploaded_file($file['tmp_name'], $target_path)) {
            // Veritabanına kaydedilecek yol (public/ ile başlayan)
            $db_photo_path = 'public/uploads/avatars/' . $file_name; 

            // Mevcut fotoğrafı sil
            $old_photo_db_path = $user['profile_photo'] ?? null;
            if ($old_photo_db_path && strpos($old_photo_db_path, 'public/uploads/avatars/') === 0) {
                // Proje kök dizinini user klasöründen bul
                $project_root = dirname(__DIR__); // /d:/ardato/devs gibi
                $old_photo_server_path = $project_root . '/' . $old_photo_db_path;
                
                // Dosya varsa ve yeni dosya ile aynı değilse sil
                if (file_exists($old_photo_server_path) && $old_photo_server_path !== $project_root . '/' . $db_photo_path) {
                   if (!unlink($old_photo_server_path)) {
                       error_log("Eski avatar silinemedi: " . $old_photo_server_path);
                   } 
                }
            }
            ...
            $stmt->bindParam(':photo', $db_photo_path); // public/ ile başlayan yolu kaydet
            $stmt->bindParam(':user_id', $user_id);

            if ($stmt->execute()) {
                $_SESSION['success_message'] = "Profil fotoğrafı başarıyla güncellendi."; // Session mesajı kullan
                // Sayfayı yeniden yükle (session mesajını göstermek için)
                header("Location: account-profile.php?updated=true");
                exit();
            } else {
                $error_message = "Profil fotoğrafı veritabanına kaydedilirken bir hata oluştu.";
                 error_log("Avatar DB güncelleme hatası: User ID " . $user_id);
            }
        } else {
            $error_message = "Dosya taşınırken bir hata oluştu (muhtemelen yazma izinleri).";
            error_log("Avatar taşınamadı: " . $target_path);
        }
    }
    ...
    $error_message = "Profil fotoğrafı yüklenirken bir hata oluştu (Hata Kodu: " . $_FILES['profile_photo']['error'] . ").";
}
